    ...
    while not isLoaded do
        ...
        if isValid then
            if IsInteriorEntitySetActive(interior, "rho_sal_tables01") then
                print("Rhodes Saloon Interior Already Active")
            else
        ...
        local interior = GetInteriorAtCoords(-1864.5424804688,-1727.5080566406,88.252014160156)
        local isValid = IsValidInterior(interior)
        if isValid then
            if IsInteriorEntitySetActive(interior, "mp006_mshine_theme_goth") then
                print("Interior Already Active")
        ...
        local interior = GetInteriorAtCoords(-397.96859741211,1729.6801757813,215.5149230957)
        local isValid = IsValidInterior(interior)
        if isValid then
            if IsInteriorEntitySetActive(interior, "che_cabin1") then
                print("Interior Already Active")
            else
        ...
        local interior = GetInteriorAtCoords(1448.0001220703,373.0,90.000007629395)
        local isValid = IsValidInterior(interior)
        if isValid then
            if IsInteriorEntitySetActive(interior, "eme_saloon_intgroup_furniture_mp") then
                print("Interior Already Active")
            else
                ActivateInteriorEntitySet(interior, "eme_saloon_intgroup_furniture_mp")
                print("Interior Activated")
            end
        end
        isLoaded = true
...
-- RequestImap(-612173099) --=2996.896484375,y=2197.3876953125,z=166.15097045898
